...
from services import MatchMessageFormatter, ProfileSubscription, HeroEmojiMapper, MatchService
...
class Matches(commands.Cog):
    ...
    async def match_preview(self, interaction: discord.Interaction) -> None:
        ...
        if self.profile_subscription and interaction.guild:
            subscribed_uuids = set(self.profile_subscription.get_profiles(interaction.guild.id))
    ...
    async def match_profile_subscribe(
        ...
        was_new = self.profile_subscription.add_profile(interaction.guild.id, formatted_uuid)
    ...
    async def match_profile_unsubscribe(
        ...
        was_removed = self.profile_subscription.remove_profile(interaction.guild.id, formatted_uuid)
    ...
    async def match_profile_list(self, interaction: discord.Interaction) -> None:
        ...
        player_uuids = self.profile_subscription.get_profiles(interaction.guild.id)
