...
async def download_all_hero_icons(output_dir: str = "hero_icons") -> None:
    ...
    for hero_data in heroes_data:
        if hero_data and hero_data.get("data") and hero_data["data"].get("icon"):
            heroes_with_icons.append(hero_data)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for hero_data in heroes_with_icons:
            hero_name = hero_data.get("name", "Unknown")
            hero_slug = hero_data.get("slug", "").lower() if hero_data.get("slug") else hero_name.lower()
            icon_asset_id = hero_data["data"]["icon"]
            display_name = hero_data["data"].get("displayName", hero_name)
